...
func (protocol *regTVProtocol) Encode(msg interface{}) (bs []byte, err error) {
	defer func() { err = commons.DCatch(recover(), err) }()
	...
	case bool, *bool:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	case []bool:
		protocol.writeType(w, REGISTER_TYPE_BOOL_ARRAY)
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]bool)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	...
	case []int8:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]int8)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	...
	case []uint8:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]uint8)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	...
	case []int16:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]int16)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	...
	case []uint16:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]uint16)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	...
	case []int32:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]int32)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	...
	case []uint32:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]uint32)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	...
	case []int64:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]int64)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))
	...
	case []uint64:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint8(len(msg.([]uint64)))))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, msg))

	case int:
		protocol.writeType(w, REGISTER_TYPE_INT)
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, int64(msg.(int))))
	case *int:
		protocol.writeType(w, REGISTER_TYPE_INT)
		i := int64(*(msg.(*int)))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, i))
	case []int:
		protocol.writeType(w, REGISTER_TYPE_INT_ARRAY)
		size := uint8(len(msg.([]int)))
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, size))
		for i := uint8(0); i < size; i++ {
			commons.PanicIfPresent(binary.Write(w, binary.BigEndian, int64(msg.([]int)[i])))
		}
	case uint:
		protocol.writeType(w, REGISTER_TYPE_UINT)
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, uint64(msg.(uint))))
	case *uint:
		protocol.writeType(w, REGISTER_TYPE_UINT)
	...
	case []uint:
		...
		commons.PanicIfPresent(binary.Write(w, binary.BigEndian, size))
		for i := uint8(0); i < size; i++ {
			commons.PanicIfPresent(binary.Write(w, binary.BigEndian, int64(msg.([]uint)[i])))
		}
	default:
...
func (protocol *regTVProtocol) Decode(c io.Reader) (interface{}, error) {
	...
	case REGISTER_TYPE_STRING_ARRAY:
		var size uint8
		commons.PanicIfPresent(binary.Read(c, binary.BigEndian, &size))
	...
	case REGISTER_TYPE_BOOL:
		var p bool
		commons.PanicIfPresent(binary.Read(c, binary.BigEndian, &p))
		return p, nil
...
func ReadString(conn io.Reader) string {
	var len uint8
	commons.PanicIfPresent(binary.Read(conn, binary.BigEndian, &len))
	if len == 0 {
		return ""
	}
	val := make([]byte, len)
...
func WriteString(w io.Writer, str string) {
	...
	commons.PanicIfPresent(binary.Write(w, binary.BigEndian, len))
	if len > 0 {
