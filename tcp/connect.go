...
import (
	...
	atomicKit "github.com/ihaiker/gokit/concurrent/atomic"
...
func (c *Connect) Write(msg interface{}) (err error) {
	...
	defer func() { err = commons.Catch(recover()) }()
...
func (c *Connect) heartbeatLoop() {
	...
	c.idleFlag = atomicKit.NewAtomicInt32(0)
	...
	for {
		...
		case <-c.closeChan:
			return
		case <-c.idleTimer.C:
			if c.idleFlag.GetAndIncrement(1) <= int32(c.config.IdleTimeout) {
